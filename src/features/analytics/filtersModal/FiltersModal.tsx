...
const FiltersModal: React.FC<TFilterListProps> = ({
  ...
  const categories = useAppSelector((state) => state.transactionsReducer.items);
  ...
  const getIndeterminate = useCallback(
    (category) => {
      return (
        ...
        filters[category]?.length !== initialFilters[category]?.length
      );
    },
    [filters, initialFilters]
  );
  console.log(Object.entries(filtersOptions));
  return (
    <>
      <Dialogue
        visible={visible}
              ...
              <View>
                ...
                <View style={style.buttons}>
                  {options
                    ...
                    .map((option) => {
                      const color = filters[label].includes(option.value)
                          ...
                          <Button
                            ...
                            styles={{
                              root: {
                                ...style.button,
                                borderWidth: 1.5,
                                borderColor: filters[label].includes(
                                  option.value
                                )
                                  ? theme.colors.independentForeground
                                  : theme.colors.pale,
                          ...
                          >
                            <View style={style.buttonContent}>
                              <SvgSelector
                                ...
                                stroke={color}
                              />
                              <Text style={[style.buttonLabel, { color }]}>
                                {option.label}
                              </Text>
                            </View>
                          </Button>
